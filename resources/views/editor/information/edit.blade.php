...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Edit Artikel Informasi</h2>
            <p class="text-sm text-gray-500 mt-1">Perbarui artikel: <strong>{{ $informationPost->title }}</strong></p>
        </div>
    ...
    <form method="POST" action="{{ route('cms.information.update', $informationPost) }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm space-y-5">
            ...
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $informationPost->title) }}" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-xl text-sm focus:ring-indigo-500 focus:border-indigo-500 @error('title') border-rose-500 @enderror" required />
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <select name="category_id" id="category_id" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-xl text-sm focus:ring-indigo-500 focus:border-indigo-500" required>
                        @foreach($categories as $cat)
                            <option value="{{ $cat->id }}" {{ old('category_id', $informationPost->category_id) == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="status" id="status" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-xl text-sm" required>
                        <option value="draft" {{ old('status', $informationPost->status) === 'draft' ? 'selected' : '' }}>Draft</option>
                        <option value="published" {{ old('status', $informationPost->status) === 'published' ? 'selected' : '' }}>Published</option>
                    </select>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="excerpt" id="excerpt" rows="3" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-xl text-sm">{{ old('excerpt', $informationPost->excerpt) }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea name="content" id="content" rows="14" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-xl text-sm font-mono @error('content') border-rose-500 @enderror" required>{{ old('content', $informationPost->content) }}</textarea>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm space-y-4">
            ...
            @if($informationPost->featured_image_url)
                <div class="p-3 bg-gray-50 border border-gray-200 rounded-xl flex items-center space-x-4">
                    <img src="{{ $informationPost->featured_image_url }}" alt="{{ $informationPost->featured_image_alt }}" class="h-16 w-24 object-cover rounded-lg border border-gray-200" />
                    <span class="text-xs text-emerald-700 font-semibold bg-emerald-50 px-2 py-1 rounded">Gambar Saat Ini</span>
                </div>
            @endif
            ...
            <div>
                ...
                <p class="text-xs text-gray-400 mt-1">Upload baru untuk mengganti gambar. Format: PNG, JPG, JPEG, WEBP. Maks. 2MB.</p>
            ...
            <div>
                ...
                <input type="text" name="featured_image_alt" id="featured_image_alt" value="{{ old('featured_image_alt', $informationPost->featured_image_alt) }}" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-xl text-sm" />
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm space-y-4">
            ...
            <div>
                ...
                <input type="text" name="meta_title" id="meta_title" value="{{ old('meta_title', $informationPost->meta_title) }}" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-xl text-sm" />
            </div>
            <div>
                <label for="meta_description" class="block text-sm font-semibold text-gray-700 mb-1">Meta Description</label>
                <textarea name="meta_description" id="meta_description" rows="3" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-xl text-sm">{{ old('meta_description', $informationPost->meta_description) }}</textarea>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end space-x-3">
            ...
            <button type="submit" class="px-5 py-2.5 bg-indigo-600 text-white rounded-xl text-sm font-medium hover:bg-indigo-700 shadow-sm transition">Perbarui Artikel</button>
        </div>
    </form>
</div>
@endsection
